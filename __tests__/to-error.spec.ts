...
import type { MongoError } from '../src/types';
...
describe('toError', () => {
  it('converts MongoDB error with all properties to JavaScript Error', () => {
    ...
    expect(result.message).toBe(
      '[11000] DuplicateKey: Duplicate key error collection | RetriableError',
    );
  ...
  it('converts MongoDB error without categories', () => {
    ...
    expect(result.message).toBe(
      '[1] InternalError: An unspecified internal error occurred.',
    );
    expect(result.cause).toBe(mongoError);
  });
  ...
  it('custom formatter receives complete MongoError object', () => {
    ...
    expect(result.message).toBe(
      'HostUnreachable (6): The host is unreachable. [NetworkError, RetriableError]',
    );
    expect(result.cause).toBe(mongoError);
  });
